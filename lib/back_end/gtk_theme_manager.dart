...
class ThemeManager {
  ...
  static Map <String, Map <String, bool>>themeSupport = {}; //Map to contain support info of theme
  ...
  List<Color> extractColorsFromFile(String filePath,{bool? demo}) {
    ...
    final colorRegExp = RegExp(r'#([0-9a-fA-F]{6}|[0-9a-fA-F]{3})|rgba?\([^)]*\)');
  ...
  Future<void> updateColors ({
    ...
    required List<int>editedIndex, bool? update})async {
    //Update colours inside a CSS code
    update ??= true;
     String cssContents = await File("$path-new").readAsString();
...
cssContents=cssContents.replaceAll(oldHex.toLowerCase(), newHex.toLowerCase());
cssContents=cssContents.replaceAll(oldHex.toUpperCase(), newHex.toUpperCase());
}
...
File cssFile = File((test ?? false)?"$path-new":path);
 ...
 if(update)   {
      ThemeDt.themeColors = ThemeDt().extractColors(filePath: cssFile.path);
